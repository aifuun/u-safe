...
**Recommendation**: Update `framework/profiles/tauri.json` to include these 11 additional rules.
...
Add missing rules to `framework/profiles/tauri.json`:
...
```bash
# Tauri verification
cat framework/profiles/tauri.json | jq '.rules.include | length'
# Expected: 34

grep -r "profiles:.*tauri" docs/ai-guides/rules/templates/ | wc -l
# Expected: 34

# Next.js-AWS verification
cat framework/profiles/nextjs-aws.json | jq '.rules.include | length'
# Expected: 43

grep -r "profiles:.*nextjs-aws" docs/ai-guides/rules/templates/ | wc -l
# Expected: 43
```
...
**Action Items**:
1. Update `framework/profiles/tauri.json` (+11 rules)
2. Verify `framework/profiles/nextjs-aws.json` (check for missing 5 rules)
