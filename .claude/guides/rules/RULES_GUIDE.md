...
depends_on:
  ...
  - .claude/guides/doc-templates/MANAGE_DOCS_GUIDE.md
---
...
7. [Migration from Deprecated Workflow](#7-migration-from-deprecated-workflow)
...
- **Maintains single source of truth** in `.claude/guides/rules/templates/`
...
- Old approach: `update-framework` → batch sync (30-50% redundant rules)
...
- [Pillar Name](../../../../.claude/pillars/pillar-file.md) - Deep dive into X
```
...
def generate_rules(profile: str, categories: list = None):
    ...
    2. Load template files from .claude/guides/rules/templates/
    ...
    # 1. Read profile
    profile_config = read_profile(f".claude/profiles/{profile}.yaml")
    tech_stack = profile_config['tech_stack']

    # 2. Load templates
    template_dirs = glob('.claude/guides/rules/templates/*/')
...
**What it syncs**:
- `.claude/guides/` directory (guides + templates)
- Including `.claude/guides/rules/templates/`
...
vi .claude/guides/rules/templates/core/error-handling.md
...
/update-framework ~/ai-dev
  → Syncs Pillars
  → Syncs Rules # Now use /manage-rules for rule generation
  → Syncs Workflow
```
...
/update-guides ~/ai-dev
  → Syncs .claude/guides/ (including rules/templates/)
...
## 7. Migration from Deprecated Workflow
...
```bash
# In framework repo
vi .claude/guides/rules/templates/architecture/clean-architecture.md

# In project repo
/update-guides ~/dev/ai-dev
  → Syncs .claude/guides/ (templates only, no generated files)
...
- **Pillars**: `.claude/pillars/` (deep technical knowledge, 200-300 lines)
