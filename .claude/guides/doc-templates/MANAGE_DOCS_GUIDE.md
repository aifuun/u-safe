...
depends_on:
  - STACK_TAGS.md
  - .claude/guides/doc-templates/README.md
---
...
def detect_project_context():
    ...
    if exists('docs/project-profile.md'):
        profile = json.load('docs/project-profile.md')['profile']
    else:
...
def process_module(module_name, profile, stacks, variables):
    ...
    guide_path = f'.claude/guides/doc-templates/{module_name}/{module_name}.guide'
    ...
    template_dir = f'.claude/guides/doc-templates/{module_name}/templates'
...
**Reference:** See `.claude/guides/doc-templates/STACK_TAGS.md` for complete specification.
...
| `{{profile}}` | `docs/project-profile.md` → `profile` | `"tauri"` |
...
**Execution:**
1. Try to read `.claude/guides/doc-templates/ops/templates/DEPLOYMENT.md.template`
...
**Steps:**
1. Update `docs/project-profile.md`:
   ```json
   {
...
def process_design_module(profile, stacks, variables):
    ...
    main_template = '.claude/guides/doc-templates/design/templates/DESIGN.md.template'
    ...
    for template_name in foundation_templates:
        template_path = f'.claude/guides/doc-templates/design/templates/ui/foundation/{template_name}'
    ...
    for template_name in component_templates:
        template_path = f'.claude/guides/doc-templates/design/templates/ui/components/{template_name}'
        content = read_file(template_path)
        content = process_stack_tags(content, stacks)
        content = substitute_variables(content, variables)
    ...
    for template_name in ux_templates:
        template_path = f'.claude/guides/doc-templates/design/templates/ux/{template_name}'
        content = read_file(template_path)
        content = process_stack_tags(content, stacks)
        content = substitute_variables(content, variables)
...
**Key takeaways:**
1. **Use unified paths** from Issue #337 (`.claude/guides/doc-templates/`)
