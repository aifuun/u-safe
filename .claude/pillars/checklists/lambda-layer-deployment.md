...
for FUNC in {{PROJECT_NAME}}-{{LAMBDA_FUNCTION}}-{{REGION}}-{{ENVIRONMENT}} {{PROJECT_NAME}}-batch-{{REGION}}-{{ENVIRONMENT}}; do
  aws lambda update-function-configuration \
    --function-name $FUNC \
    --layers arn:aws:lambda:us-east-1:696249060859:layer:{{PROJECT_NAME}}-shared-{{ENVIRONMENT}}:$LAYER_VERSION \
    --profile dev
  echo "✅ Updated $FUNC"
done
...
aws lambda get-function-configuration \
  --function-name {{PROJECT_NAME}}-{{LAMBDA_FUNCTION}}-{{REGION}}-{{ENVIRONMENT}} \
  --profile dev | jq '.Layers[0].Arn'
# ✓ 期望看到: arn:aws:lambda:us-east-1:696249060859:layer:{{PROJECT_NAME}}-shared-{{ENVIRONMENT}}:$LAYER_VERSION
```
...
- [ ] **检查 CloudWatch 日志是否反映新代码**
  ```bash
  aws logs tail /aws/lambda/{{PROJECT_NAME}}-{{LAMBDA_FUNCTION}}-{{REGION}}-{{ENVIRONMENT}} \
...
4. 检查 Layer 是否发布了新版本
   aws lambda list-layer-versions --layer-name {{PROJECT_NAME}}-shared-{{ENVIRONMENT}} --profile dev
...
| 查看当前版本 | `aws lambda get-function-configuration --function-name {{PROJECT_NAME}}-{{LAMBDA_FUNCTION}}-{{REGION}}-{{ENVIRONMENT}} --profile dev \| jq '.Layers'` |
| 查看所有版本 | `aws lambda list-layer-versions --layer-name {{PROJECT_NAME}}-shared-{{ENVIRONMENT}} --profile dev` |
