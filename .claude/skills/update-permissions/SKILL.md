...
**工作流序列**：
```
python3 scripts/init-project.py --profile=tauri --name=my-app  # 初始化项目
...
if not os.path.exists(target_dir):
    ...
    print("请先运行: python3 scripts/init-project.py")
    sys.exit(1)
```
...
if [ ! -d "$target_dir" ]; then
    ...
    echo "请先运行: python3 scripts/init-project.py"
    exit 1
fi
```
...
解决:
  1. 先初始化项目: python3 scripts/init-project.py
  2. 或检查目标路径是否正确
```
...
请先运行: python3 scripts/init-project.py
```
